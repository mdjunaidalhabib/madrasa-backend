...
async function makeUniqueSlug(base: string) {
  const [rows]: any = await db.query("SELECT slug FROM madrasas WHERE slug LIKE ?", [`${base}%`]);
...
export const listMadrasas = async (req: Request, res: Response) => {
  try {
    ...
    const [rows]: any = await db.query(
      `
      SELECT 
        m.id, m.name, m.slug, m.address, m.phone, m.student_limit, m.user_limit,
        m.is_active, m.website_status,
        s.plan_id,
        p.name AS plan_name,
        s.start_date,
        s.end_date
...
export const assignPlanToMadrasa = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!madrasaId) return res.status(400).json({ message: "Invalid madrasa id" });
    ...
    if (mRows[0].deleted_at) {
      await conn.rollback();
      return res.status(400).json({ message: "Cannot assign plan to trashed madrasa" });
    }
    ...
    await conn.query(`UPDATE madrasa_subscriptions SET is_active=0 WHERE madrasa_id=?`, [
      madrasaId,
    ]);
    ...
    await conn.query(`UPDATE madrasas SET student_limit=?, user_limit=? WHERE id=?`, [
      plan.student_limit,
      plan.user_limit,
      madrasaId,
    ]);
  ...
  try {
    ...
    const [madrasaResult]: any = await conn.query(
      `INSERT INTO madrasas 
       ...
       VALUES (?, ?, ?, ?, ?, ?, 1)`,
      [name, address || null, phone || null, finalSlug, student_limit || 100, user_limit || 5],
    );
    ...
    if (divisionIds.length > 0) {
      ...
      await conn.query(`UPDATE madrasa_divisions SET is_active=1 WHERE madrasa_id=?`, [madrasaId]);
    }
    ...
    if (moduleIds.length > 0) {
      ...
      await conn.query(`UPDATE madrasa_modules SET is_active=1 WHERE madrasa_id=?`, [madrasaId]);
    }
    ...
    if (classIds.length > 0) {
      ...
      await conn.query(`UPDATE madrasa_classes SET is_active=1 WHERE madrasa_id=?`, [madrasaId]);
    }
    ...
    if (bookIds.length > 0) {
      ...
      await conn.query(`UPDATE madrasa_books SET is_active=1 WHERE madrasa_id=?`, [madrasaId]);
    }
...
export const updateMadrasa = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!id) return res.status(400).json({ message: "Invalid madrasa id" });
    if (website_status && !["active", "limited", "disabled"].includes(String(website_status))) {
    ...
    await conn.query(
      ...
      [
        ...
        address || null,
        phone || null,
        Number(student_limit) || null,
        Number(user_limit) || null,
    ...
    if (plan_id) {
      ...
      const plan = planRows[0];
      await conn.query(`UPDATE madrasa_subscriptions SET is_active=0 WHERE madrasa_id=?`, [id]);
      await conn.query(
        `INSERT INTO madrasa_subscriptions (madrasa_id, plan_id, start_date, end_date, is_active) VALUES (?, ?, CURDATE(), DATE_ADD(CURDATE(), INTERVAL ? DAY), 1)`,
    ...
    await conn.query(
      `INSERT INTO activity_logs (madrasa_id, action, entity, entity_id, details) VALUES (?, 'SUPER_ADMIN_MADRASA_UPDATED', 'madrasa', ?, ?)`,
      [id, id, JSON.stringify({ name, slug, website_status, plan_id })],
    );
...
export const activateMadrasa = async (req: Request, res: Response) => {
  await db.query(`UPDATE madrasas SET is_active=1 WHERE id=? AND deleted_at IS NULL`, [
    req.params.id,
  ]);
...
export const suspendMadrasa = async (req: Request, res: Response) => {
  await db.query(`UPDATE madrasas SET is_active=0 WHERE id=? AND deleted_at IS NULL`, [
    req.params.id,
  ]);
...
export const getSuperAdminStats = async (_req: Request, res: Response) => {
  ...
  const [[totalMadrasas]]: any = await db.query("SELECT COUNT(*) as total FROM madrasas");

  const [[students]]: any = await db.query("SELECT COUNT(*) as total FROM students");
...
export const permanentDeleteMadrasa = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await conn.query("DELETE FROM madrasa_subscriptions WHERE madrasa_id=?", [id]);
