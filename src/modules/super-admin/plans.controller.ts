...
export const createPlanAdmin = async (req: Request, res: Response) => {
  try {
    ...
    if (student_limit < 0)
      return res.status(400).json({ message: "Student limit 0 বা তার বেশি হতে হবে" });

    if (user_limit < 0)
      return res.status(400).json({ message: "User limit 0 বা তার বেশি হতে হবে" });

    if (duration_days <= 0)
      return res.status(400).json({ message: "Duration days 1 বা তার বেশি হতে হবে" });

    if (price < 0) return res.status(400).json({ message: "Price 0 বা তার বেশি হতে হবে" });
...
export const updatePlanAdmin = async (req: Request, res: Response) => {
  try {
    ...
    if (r.affectedRows === 0) {
      return res.status(404).json({ message: "Plan পাওয়া যায়নি / trash এ আছে" });
    }
...
export const deletePlanAdmin = async (req: Request, res: Response) => {
  try {
    ...
    if (r.affectedRows === 0) {
      return res.status(404).json({ message: "Plan পাওয়া যায়নি / আগেই trash এ" });
    }
...
export const restorePlanAdmin = async (req: Request, res: Response) => {
  try {
    ...
    if (r.affectedRows === 0) {
      return res.status(404).json({ message: "Plan trash এ নেই / পাওয়া যায়নি" });
    }
