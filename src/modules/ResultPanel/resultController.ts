...
const getSchoolId = (req: Request) => {
  return Number(
    (req as any)?.tenant?.madrasa_id ||
      (req as any)?.user?.madrasa_id ||
...
const loadSettings = async (school_id: number) => {
  const [rows]: any = await db.query(`SELECT name, value FROM settings WHERE school_id=?`, [
    school_id,
  ]);
...
const getOrCreateSessionId = async (school_id: number, exam_id: number, class_id: number) => {
...
export const processResult = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const sorted = [...marks].sort((a: any, b: any) => Number(b.total) - Number(a.total));
    ...
    await conn.query(`DELETE FROM results_summary WHERE result_master_id=?`, [result_master_id]);
    ...
    await conn.query(`UPDATE results_master SET status='DRAFT' WHERE id=?`, [result_master_id]);
...
export const publishResult = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await db.query(`UPDATE results_master SET status='PUBLISHED' WHERE id=?`, [result_master_id]);
...
export const deleteResult = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await conn.query(`DELETE FROM results_summary WHERE result_master_id=?`, [id]);
...
export const getFullResultView = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      result_master_id,
      books: Array.from(booksMap.values()).sort((a: any, b: any) => a.book_id - b.book_id),
